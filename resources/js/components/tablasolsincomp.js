...
function createData(ci,nombre,appaterno,apmaterno) {
  
  return { ci,nombre,appaterno,apmaterno};
}
...
const rows = [
  { id: 'habilitado', numeric: false, disablePadding: false, label: '' },
...
function EnhancedTable() {
  ...
  React.useEffect(()=>{
    const fetchdata=async ()=>{
      const res=await axios(`/api/soldadosdelcuartel/${window.location.pathname.split('/')[3]}`);

      setdatax(res.data);
    };
    fetchdata();
  },[`api/soldadosdelcuartel/${window.location.pathname.split('/')[3]}`]);
      
  for (let i = 0; i < datax.length; i++) {
    data.push(createData(datax[i]['CI'],datax[i]['Nombre'],datax[i]['ApPaterno'],datax[i]['ApMaterno']));
  ...
  function guardardata(event) {
    ...
    for (let i = 0; i < selected.length; i++) {
        var data={
            CI:selected[i],
            IdCompania:window.location.pathname.split('/')[4],
        };
        axios.put('/api/aniadiracompania/',data);
